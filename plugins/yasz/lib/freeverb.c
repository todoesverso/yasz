...
void
freeverb_setwet(FREEVERB* p, float wet) {
    p->wet = wet * FV_SCALEWET;
    freeverb_update(p);
}

void
freeverb_setdry(FREEVERB* p, float dry) {
    p->dry = dry * FV_SCALEDRY;
}

void
freeverb_setwidth(FREEVERB* p, float width) {
    p->width = width;
    freeverb_update(p);
}

void
freeverb_setroomsize(FREEVERB* p, float roomsize) {
    p->roomsize = (roomsize * FV_SCALEROOM) + FV_OFFSETROOM;
    freeverb_update(p);
}

void
freeverb_setdamp(FREEVERB* p, float damp) {
    p->damp = damp * FV_SCALEDAMP;
    freeverb_update(p);
}

void
freeverb_setsr(FREEVERB* p, uint32_t sr) {
    const uint8_t combs[] = {
...
void
freeverb_tick(FREEVERB* p, double inputL, double inputR) {
    double outL = 0.0f;
    double outR = 0.0f;
    uint8_t i = 0;
