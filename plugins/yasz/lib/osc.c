...
static double
osc_calc_new_phase_rt(OSC* p_osc) {
    return p_osc->twopioversr * p_osc->freq;
}

/* WAVE types */
static double
osc_sine_rt(OSC* p_osc) {
 ...
 *************************/
void
osc_init_rt(OSC* p_osc, uint32_t srate) {
    p_osc->freq = 0.0f;
    p_osc->phase = 0.0f;
    p_osc->phaseinc = 0.0f;
    p_osc->srate = srate;
...
void
osc_update_freq_rt(OSC* p_osc, double newfreq) {
    p_osc->freq = newfreq;
    p_osc->phaseinc = osc_calc_new_phase_rt(p_osc);
}

void
osc_update_phase_rt(OSC* p_osc, double phase) {
    p_osc->phase = phase;
}

void
osc_update_wavetype_rt(OSC* p_osc, uint8_t wavetype) {
    p_osc->wavetype = wavetype;
}

void
osc_update_srate_rt(OSC* p_osc, uint32_t srate) {
    p_osc->srate = srate;
    p_osc->twopioversr = (double)TWO_PI / srate;  // NOLINT(readability/casting)
}

double
osc_get_out_rt(OSC* p_osc) {
    double val;
